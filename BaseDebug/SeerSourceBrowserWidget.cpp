...
void SeerSourceBrowserWidget::handleText (const QString& text) {
    ...
    if (text.startsWith("^done,files=[") && text.endsWith("]")) {
        ...
        QString files_text     = SDS::parseFirst(text, "files=", '[', ']', false);
        QStringList files_list = SDS::parse(files_text, "", '{', '}', false);
        ...
        for ( const auto& entry_text : files_list  ) {
            ...
            QString file_text     = SDS::parseFirst(entry_text, "file=",     '"', '"', false);
            QString fullname_text = SDS::parseFirst(entry_text, "fullname=", '"', '"', false);
